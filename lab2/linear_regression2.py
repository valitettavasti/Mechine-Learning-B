...
X = np.ones((np.size(load_data_training, 0), 2))
...
plt.plot(x, w[0] * x + w[1], label='拟合曲线')
plt.xlabel('x')
plt.ylabel('y')
plt.legend()
plt.show()
MSE = np.sum(np.square(load_data_testing[:, 1] - (load_data_testing[:, 0] * w[0] + w[1]))) / np.size(load_data_training,
                                                                                                     0)
print('均方误差:',MSE)
